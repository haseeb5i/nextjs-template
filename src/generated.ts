...
export const LaunchesDocument = `
  ...
  launchesPast(limit: $limit) {
    ...
    `;
export const useLaunchesQuery = <TData = LaunchesQuery, TError = unknown>(
  variables?: LaunchesQueryVariables,
  options?: UseQueryOptions<LaunchesQuery, TError, TData>
) =>
  useQuery<LaunchesQuery, TError, TData>(
    variables === undefined ? ["Launches"] : ["Launches", variables],
    fetcher<LaunchesQuery, LaunchesQueryVariables>(LaunchesDocument, variables),
    options
  );
useLaunchesQuery.document = LaunchesDocument;

useLaunchesQuery.getKey = (variables?: LaunchesQueryVariables) =>
  variables === undefined ? ["Launches"] : ["Launches", variables];
useLaunchesQuery.fetcher = (variables?: LaunchesQueryVariables) =>
  fetcher<LaunchesQuery, LaunchesQueryVariables>(LaunchesDocument, variables);
